...
class TFProcess:
    ...
    def init_net(self, next_batch):
        ...
        self.y_conv, self.z_conv = self.net.construct_net(self.x)
...
class Net:
    def __init__(self, cfg, training, weights):
        ...
        self.BOTTLENECK_SIZE = self.cfg['model']['bottleneck']
        self.no_bottleneck = (self.RESIDUAL_FILTERS == self.BOTTLENECK_SIZE)
    ...
    def get_block_key(self):
        return "blk" + str(self.batch_norm_count)

    def conv2d(self, x, filter_size, input_channels, output_channels):
        weight_key = self.get_block_key()
        conv_key = weight_key + "/conv_weight"
        W_conv = weight_variable([filter_size, filter_size, input_channels,
                                  output_channels], name=conv_key)
        self.weights.append(W_conv)
        with tf.variable_scope(weight_key):
            return tf.nn.conv2d(x, W_conv, data_format='NCHW',
                                strides=[1, 1, 1, 1], padding='SAME')

    def batch_norm(self, x, apply_relu=False):
        weight_key = self.get_block_key()
        with tf.variable_scope(weight_key):
            outputs = tf.layers.batch_normalization(
                x, epsilon=1e-5, axis=1, fused=True,
    ...
    def fully_connected(self, x, input_dim, output_dim, name):
        weight_name = name + '/weight'
        bias_name = name + '/bias'
        W_fc1 = weight_variable([input_dim, output_dim], name=weight_name)
        b_fc1 = bias_variable([output_dim], name=bias_name)
        self.weights.append(W_fc1)
        self.weights.append(b_fc1)
        return tf.matmul(x, W_fc1) + b_fc1

    def conv_block(self, inputs, filter_size, channels_in, channels_out=None, skip=None):
        if channels_out is None:
            channels_out = channels_in
        self.new_block_key()
        if self.PREACT:
            outputs = self.batch_norm(inputs, apply_relu=True)
            outputs = self.conv2d(outputs, filter_size, channels_in, channels_out)
            if skip is not None:
                outputs += skip
        else:
            outputs = self.conv2d(inputs, filter_size, channels_in, channels_out)
            if skip is None:
                outputs = self.batch_norm(outputs, apply_relu=True)
            else:
                outputs = self.batch_norm(outputs, apply_relu=False)
                outputs = tf.nn.relu(outputs + skip)
        return outputs

    def std_block(self, inputs, channels):
        outputs = self.conv_block(inputs, 3, channels)
        return self.conv_block(outputs, 3, channels, skip=inputs)

    def residual_block(self, inputs):
        if self.no_bottleneck:
            outputs = self.std_block(inputs, self.RESIDUAL_FILTERS)
        else:
            outputs = self.conv_block(inputs, 1, self.RESIDUAL_FILTERS,
                                      channels_out=self.BOTTLENECK_SIZE)
            outputs = self.std_block(outputs, self.BOTTLENECK_SIZE)
            if self.MUTIPLE_CONV:
                outputs = self.std_block(outputs, self.BOTTLENECK_SIZE)
            outputs = self.conv_block(outputs, 1, self.BOTTLENECK_SIZE,
                                      channels_out=self.RESIDUAL_FILTERS)
        return outputs
    ...
    def construct_net(self, planes):
        ...
        # Input convolution
        flow = self.conv_block(x_planes, 3, 112, channels_out=self.RESIDUAL_FILTERS)
        # Residual tower
        for _ in range(0, self.RESIDUAL_BLOCKS):
            flow = self.residual_block(flow)

        # Policy head
        conv_pol = self.conv_block(flow, 1, self.RESIDUAL_FILTERS, channels_out=32)
        h_conv_pol_flat = tf.reshape(conv_pol, [-1, 32*8*8])
        h_fc1 = self.fully_connected(h_conv_pol_flat, 32*8*8, 1858, name='fc1')
        h_fc1 = tf.identity(h_fc1, name='policy_head')

        # Value head
        conv_val = self.conv_block(flow, 1, self.RESIDUAL_FILTERS, channels_out=32)
        h_conv_val_flat = tf.reshape(conv_val, [-1, 32 * 8 * 8])
        h_fc2 = self.fully_connected(h_conv_val_flat, 32 * 8 * 8, 128, name='fc2')
        h_fc2 = tf.nn.relu(h_fc2)
        h_fc3 = self.fully_connected(h_fc2, 128, 1, name='fc3')
        h_fc3 = tf.nn.tanh(h_fc3, name='value_head')
